...
for (i in 1:num_frames) {
  ...
  x <- 0:20
  t <- seq(0, 1, 0.005)
  p <- u * k_param[i] / (u * k_param[i] + v)
  binomial_pdf <- dbinom(x, size = 20, prob = p)
  betabinomial_pdf <- dbetabinom(x, 20, u * k_param[i], v)
  beta_pdf <- dbeta(t, shape1 = u * k_param[i], shape2 = v)
...
output_video <- "comparison_video_alpha.mp4"
